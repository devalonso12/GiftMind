...
import { Search, Loader2, ArrowLeft, Wallet, AtSign, Sparkles, TrendingUp, Plus, X, Check, Zap, AlertCircle } from 'lucide-react';
import { Input } from '../ui/input';
import { Label } from '../ui/label';

function SocialLogo({ platform }: { platform: string }) {
  const name = platform === 'twitter' ? 'x' : platform;
  return <img src={`/logos/social/${name}.svg`} alt={platform} className="w-4 h-4 object-contain" />;
}

const PLATFORMS = [
  { value: 'twitter', label: 'X (Twitter)', icon: SocialLogo, color: 'text-slate-300', bg: 'bg-slate-500/10', border: 'border-slate-500/20' },
  { value: 'snapchat', label: 'Snapchat', icon: SocialLogo, color: 'text-yellow-400', bg: 'bg-yellow-500/10', border: 'border-yellow-500/20' },
  { value: 'tiktok', label: 'TikTok', icon: SocialLogo, color: 'text-pink-400', bg: 'bg-pink-500/10', border: 'border-pink-500/20' },
  { value: 'instagram', label: 'Instagram', icon: SocialLogo, color: 'text-pink-400', bg: 'bg-pink-500/10', border: 'border-pink-500/20' },
  { value: 'farcaster', label: 'Farcaster', icon: SocialLogo, color: 'text-violet-400', bg: 'bg-violet-500/10', border: 'border-violet-500/20' },
];
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (platform === 'twitter') {
        ...
        profiles.push(
          generateProfile('', ['Snapchat', 'AR', 'Creator'], 91),
          generateProfile('_streak', ['Social', 'Influencer'], 84),
        );
      } else if (platform === 'tiktok') {
        profiles.push(
          generateProfile('', ['TikTok', 'Viral', 'Content'], 93),
          generateProfile('_official', ['Creator', 'Trending'], 86),
          generateProfile('_live', ['Streaming', 'Community'], 78),
        );
...
export function SearchStep() {
  ...
  const hasSearchData = socialSearches.length > 0 || Boolean(walletSearchQuery.trim() && !walletError);
  const canAnalyze = hasSearchData && senderWallet?.connected;
  ...
  return (
    <div className="animate-slide-up">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 text-sm">Provide a wallet or social handle. Link-only gifts bind to the claimant wallet.</p>
      </div>
      ...
      <div className="surface-stellar-strong rounded-2xl p-6 space-y-6">
        ...
        <div className="space-y-3">
          <Label className="text-xs font-semibold uppercase tracking-wider text-amber-400/80 flex items-center gap-2">
            <Wallet className="h-3.5 w-3.5" />Recipient Wallet (Optional)
          </Label>
              ...
              <div className="space-y-1.5 mt-2">
                ...
                <div className="flex items-center gap-3">
                       <div className="w-8 h-8 rounded-lg bg-slate-800/50 flex items-center justify-center">
                         <SocialLogo platform={result.platform} />
                       </div>
                      <div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {socialSearches.map((search, idx) => {
                    return (
                      <div key={idx} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800/30 border border-slate-700/50 group">
                        <SocialLogo platform={search.platform} />
                        <span className="text-sm font-medium text-slate-300">@{search.handle}</span>
        ...
        {!hasSearchData && (
          <div className="p-4 rounded-xl surface-void border border-slate-700/50">
            <p className="text-xs text-slate-500">Add a wallet or at least one social profile to analyze.</p>
          </div>
        )}

        {/* Actions */}
        <div className="flex gap-3">
          <button onClick={() => setStep('relationship')} disabled={isLoading} className="flex-1 h-11 rounded-xl surface-stellar text-slate-500 hover:text-slate-300 flex items-center justify-center gap-2 transition-all">
        ...
        {!senderWallet?.connected && (
          <p className="text-[10px] text-red-400 mt-2">A connected wallet is required to analyze and save insights. Please connect your wallet.</p>
        )}
      </div>
    </div>
  );
