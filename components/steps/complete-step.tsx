...
export function CompleteStep() {
  ...
  const shareGift = async () => {
      if (navigator.share) {
        const shareText = giftType === 'NFT' ? 'Someone sent you a personalized NFT!' : `Someone sent you ${amount} SOL!`;
        try { await navigator.share({ title: 'GM Gift!', text: shareText, url: claimUrl }); }
        catch { copyLink(); }
      } else { copyLink(); }
  };
      ...
      <div className="surface-stellar-strong rounded-2xl p-6 space-y-6">
        <div className="relative p-8 rounded-2xl overflow-hidden depth-4d text-center">
          ...
          <div className="relative">
            <img src="/favicon.svg" alt="GM" className="h-8 w-8 mx-auto mb-3 object-contain" />
            <p className="text-4xl font-black text-solar">{giftType === 'NFT' ? '1 NFT' : `${amount} SOL`}</p>
